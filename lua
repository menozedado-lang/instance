local function instanceShowLoadingNotification()
	...
	local conn = RunService.RenderStepped:Connect(function(dt)
		...
		label.Text = "cracked by U" .. string.rep(".", dotIndex)
	end)
	return function()
...
DragPreview.Size = Instance.Size;
            ...
            while (InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or #InputService:GetTouches() > 0) do
        ...
        function KeyPicker:GetState()
            ...
            if KeyPicker.Mode == 'Hold' then
                ...
                if Key == 'MB1' or Key == 'MB2' then
                    return (Key == 'MB1' and (InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or #InputService:GetTouches() > 0)) or (Key == 'MB2' and InputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton2))
                else
        ...
        PickOuter.InputBegan:Connect(function(Input)
            if (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) and not Library:MouseIsOverOpenedFrame() then
                ...
                Event = InputService.InputBegan:Connect(function(Input)
                    ...
                    elseif (Input.UserInputType == Enum.UserInputType.MouseButton1 or Input.UserInputType == Enum.UserInputType.Touch) then Key = 'MB1'
...
function Tab:HideTab()
    ...
    function getgenv().Library:Toggle()
            if Fading then
                return
            end
...
do
	...
	RunService.Heartbeat:Connect(function()
		if silentAim.enabled then
			if silentAim.autoShoot or (UserInputService:IsMouseButtonPressed(Enum.UserInputType.MouseButton1) or #UserInputService:GetTouches() > 0) then
				firesilent()
			end
		end
	end)
    ...
    btn.InputEnded:Connect(function(input)
        if input.UserInputType == Enum.UserInputType.Touch or input.UserInputType == Enum.UserInputType.MouseButton1 then
            if tick() - clickStart < 0.25 then
                if getgenv().Library and getgenv().Library.Toggle then
                    getgenv().Library:Toggle()
                end
            end
        end
    end)
