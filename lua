...
Modules.ChangeColor = function()
    ...
    local function applyDevConsoleRichText(root)
    ...
    runService.Heartbeat:Connect(function()
        ...
        applyDevConsoleRichText(coreGui:FindFirstChild("DevConsoleMaster"))
    end)
end

Modules.print = function(color, text)
    if not Modules.Colors[color] then 
        return 
    end 
    local Text = '<font color="rgb(' .. Modules.Colors[color] .. ')">' .. tostring(text) .. '</font>'
    print(Text)
end

Modules.printinline = function(...)
    local result = ""
    for i, v in ipairs({...}) do
        local color = v[1]
        local text = v[2]
        
        if Modules.Colors[color] then
            result = result .. '<font color="rgb(' .. Modules.Colors[color] .. ')">' .. tostring(text) .. '</font>'
        else
...
Modules.print("Purple", " _____ _____ _____ _____ _____ _____ _____ _____ ")
Modules.print("Purple", "|     |   | |   __|_   _|  _  |   | |     |   __|")
Modules.print("Purple", "|-   -| | | |__   | | | |     | | | |   --|   __|")
Modules.print("Purple", "|_____|_|___|_____| |_| |__|__|_|___|_____|_____|")
print("")

Modules.printinline(
    {"Purple", "[+] "},
    {"Green", "Loaded AC Bypass"}
)
...
local Library = loadstring(game:HttpGet('https://pastebin.com/raw/2RMiBRR5'))() 
local SaveManager = loadstring(game:HttpGet('https://pastefy.app/TcRas1ug/raw'))()
local ThemeManager = loadstring(game:HttpGet('https://pastefy.app/zqRrE8kC/raw'))()
...
local function ensureInstanceMenuCursor()
    ...
    if mc.cursor then
        pcall(function() mc.cursor:Remove() end)
    end
    if mc.outline then
        pcall(function() mc.outline:Remove() end)
    end
...
RunService.RenderStepped:Connect(function()
    ...
    local accent = (Library and Library.AccentColor) or Color3.fromRGB(0, 85, 255)
...
local weaponNameCache = {name = nil, updatedAt = 0, item = nil}

local function getCachedWeaponName(forceRefresh)
    local now = tick()
    local fighter = localfighter or local_fighter or localFighter
    local item = fighter and fighter.EquippedItem

    local itemChanged = item ~= weaponNameCache.item
    if not forceRefresh and not itemChanged and weaponNameCache.updatedAt > 0 and now - weaponNameCache.updatedAt < 0.1 then
        return weaponNameCache.name
    end

    if itemChanged then
        weaponNameCache.item = item
    end

    if item then
        local name = item:Get("Name") or item.Name
        if type(name) ~= "string" or name == "" then
            name = item:Get("WeaponName") or item:Get("DisplayName") or item:Get("ItemName")
        end

        if type(name) == "string" and name ~= "" then
            weaponNameCache.name = name
            weaponNameCache.updatedAt = now
            return name
        end
    end
    ...
    weaponNameCache.name = nil
    weaponNameCache.updatedAt = now
    return nil
end

local function curweap2()
    if getCachedWeaponName then
        local ok, weapon = pcall(getCachedWeaponName)
    ...
    for _, child in ipairs(firstPerson:GetChildren()) do
        local parts = {}
        for part in tostring(child.Name or ""):gmatch("[^-]+") do
...
local silentFOVCfg = {
    ...
    FilledEnabled       = false,
    FilledColor1        = Color3.fromRGB(119, 0, 255),
...
silentcustomization:AddToggle("SilentFOVFilled", {
    ...
    Default  = Color3.fromRGB(119, 0, 255),
    Title    = "fill color 1",
...
aimbotCustomTab:AddToggle("AimbotFOVFilled", {
    ...
    Default=Color3.fromRGB(119,0,255), Title="fill color 1",
...
local function toggletattribute(attribute, value)
...
LeftGroup:AddToggle("NoCooldown", {
    ...
    Callback = function(val)
        if val then
            toggletattribute("ShootCooldown", 0)
        end
    end
})
...
LeftGroup:AddToggle("NoSpread", {
    ...
    Callback = function(val)
        if val then
            toggletattribute("ShootSpread", 0)
        end
    end
})
...
LeftGroup:AddToggle("NoRecoil", {
    ...
    Callback = function(val)
        if val then
            toggletattribute("ShootRecoil", 0)
        end
    end
})
...
LeftGroup:AddToggle("MaxAccuracy", {
    ...
    Callback = function(val)
        if val then
            toggletattribute("ShootAccuracy", 0)
        end
    end
})
...
local meeleGroup = Tabs.Combat:AddRightGroupbox("meele")

meeleGroup:AddToggle("RapidAttack", {
    Text = "rapid attack",
    Default = false,
    Callback = function(val)
        if val then
            toggletattribute("AttackCooldown", 0)
        end
    end
})

meeleGroup:AddToggle("RapidDeflect", {
    Text = "rapid deflect",
    Default = false,
    Callback = function(val)
        if val then
            toggletattribute("DeflectCooldown", 0)
        end
    end
})

meeleGroup:AddToggle("RapidDash", {
    Text = "rapid dash",
    Default = false,
    Callback = function(val)
        if val then
            toggletattribute("DashCooldown", 0)
        end
    end
})

meeleGroup:AddToggle("InfiniteDoubleJump", {
    Text = "infinite double jump",
    Default = false,
    Callback = function(val)
        if val then
            toggletattribute("MaxDoubleJumps", 99999)
        end
    end
})
end
...
local fontok, fonterr = pcall(function()
    local fontid = "fortnite_font.otf"
    if not isfile(fontid) then
        writefile(fontid, game:HttpGet("https://github.com/nyrus573l/esp-fonts/raw/refs/heads/main/fortnite.otf"))
    end
    if isfile("fortnite_font.font") then delfile("fortnite_font.font") end
    local fontdata = {
        name = "FortniteFont",
        faces = {{ name = "Regular", weight = 400, style = "normal", assetId = getcustomasset(fontid) }}
    }
    writefile("fortnite_font.font", HttpService:JSONEncode(fontdata))
    fonts.main = Font.new(getcustomasset("fortnite_font.font"))
end)

if not fontok then
    warn("fortnite font failed to load:", fonterr)
...
function getDamageBillboardInfo(obj)
...
local gunOutlineColor1 = Color3.fromRGB(255, 0, 0)
local gunOutlineColor2 = Color3.fromRGB(0, 255, 0)
local gunOutlineColor3 = Color3.fromRGB(0, 0, 255)
local armOutlineColor1 = Color3.fromRGB(255, 0, 0)
local armOutlineColor2 = Color3.fromRGB(0, 255, 0)
local armOutlineColor3 = Color3.fromRGB(0, 0, 255)

local gunMaterial = Enum.Material.Neon
local armMaterial = Enum.Material.Neon

local gunColor1 = Color3.fromRGB(255, 0, 0)
local gunColor2 = Color3.fromRGB(0, 255, 0)
local gunColor3 = Color3.fromRGB(0, 0, 255)

local armColor1 = Color3.fromRGB(255, 0, 0)
local armColor2 = Color3.fromRGB(0, 255, 0)
local armColor3 = Color3.fromRGB(0, 0, 255)
...
local function saveorigprops(descendant)
    if not origprops[descendant] then
        origprops[descendant] = {Material = descendant.Material, Color = descendant.Color, Transparency = descendant.Transparency, textures = {}}
...
local function restoreorigprops(descendant)
    if origprops[descendant] then
        descendant.Material = origprops[descendant].Material
        descendant.Color = origprops[descendant].Color
        descendant.Transparency = origprops[descendant].Transparency
        for child, transparency in pairs(origprops[descendant].textures) do
            if child and child.Parent then child.Transparency = transparency end
        end
    end
end
...
local function viewModelEffectsActive()
    return gunChamsEnabled or armChamsEnabled or gunOutlineEnabled or armOutlineEnabled or disableArmsEnabled
end
...
local function isArmChamPart(part, model)
    ...
    if lower == "leftarm" or lower == "rightarm" then
        return true
    end
    ...
    if lower:find("hand", 1, true) and not lower:find("handguard", 1, true) and not lower:find("handle", 1, true) then
        return true
    end
    return false
end
...
local function stopchamloop()
    ...
    for part, _ in pairs(origprops) do
        if part and part.Parent then restoreorigprops(part) end
    end
    origprops = {}
end
...
applyChams = function(dt)
    ...
    for _, model in ipairs(fp:GetChildren()) do
        if model:IsA("Model") then
            local modelFillColor = isArmViewModel(model) and armColor1 or gunColor1
            local modelOutlineColor = isArmViewModel(model) and armOutlineColor1 or gunOutlineColor1

            for _, descendant in ipairs(model:GetDescendants()) do
                if descendant:IsA("BasePart") and not OUTLINE_EXCLUDED_PARTS[descendant.Name] then
                    local isArmPart = isArmChamPart(descendant, model)
                    ...
                    if disableArmsEnabled and isArmPart then
                        ...
                        local useChams = (isArmPart and armChamsEnabled) or ((not isArmPart) and gunChamsEnabled)
                        local useOutline = (isArmPart and armOutlineEnabled) or ((not isArmPart) and gunOutlineEnabled)
                        local outlineColor = isArmPart and armOutlineColor1 or gunOutlineColor1

                        if useChams then
                            descendant.Material = isArmPart and armMaterial or gunMaterial
                            descendant.Color = modelFillColor
                            for _, child in ipairs(descendant:GetChildren()) do
                                if child:IsA("Texture") or child:IsA("Decal") or child:IsA("SurfaceAppearance") then
                                    child.Transparency = 1
                                end
...
local tracerEffectOriginalPlay = nil
...
local function maketracer(pos3, endPos)
    ...
    local curtime = tick()
    if lastTracerCreations[posKey] and (curtime - lastTracerCreations[posKey]) < 0.1 then
        return  
    end
    lastTracerCreations[posKey] = curtime
    ...
    if tracerStyle == "Line" then
        ...
        for i = 1, numSegments do
            local segStart = pos3 + (direction.Unit * ((i - 1) * segmentLength))
            local segEnd = pos3 + (direction.Unit * math.min(i * segmentLength, distance))
        ...
        attachment0.WorldPosition = pos3
...
local function updtracers()
    ...
    for i = #tracers, 1, -1 do
        ...
        local age = curtime - tracer.CreatedTime
        
        if age >= tracer.Lifetime then
            if tracer.IsLine and tracer.Segments then
                for _, segment in ipairs(tracer.Segments) do
                    if segment.Outline then segment.Outline:Remove() end
                    if segment.Line then segment.Line:Remove() end
                end
            ...
            if tracer.IsLine and tracer.Segments then
                for _, segment in ipairs(tracer.Segments) do
                    ...
                    local success = pcall(function()
                        ...
                        local segmentStart, onScreenStart = worldToScreen(segment.StartPos, camera)
                        local segmentEnd, onScreenEnd = worldToScreen(segment.EndPos, camera)
                        
                        if not onScreenStart or not onScreenEnd then return end
                    ...
                    if success and shouldShow and fromPos and toPos then
                        segment.Outline.From = fromPos
                        segment.Outline.To = toPos
                        segment.Outline.Transparency = fadeAlpha
...
local function tracers2()
    ...
    pcall(function()
        ...
        TracerEffect.Play = function(self, tracerData, config, extraData)
            if not tracerEnabled then return origplay(self, tracerData, config, extraData) end
                ...
                if muzzlePos then
                    for _, hit in ipairs(tracerData.RaycastResults) do
                        if hit.Position then
                            maketracer(muzzlePos, hit.Position)
                        end
                    end
                end
            end
    ...
    tracerconn = game:GetService("RunService").RenderStepped:Connect(updtracers)
end
            ...
            if tracer.IsLine and tracer.Segments then
                for _, segment in ipairs(tracer.Segments) do
                    if segment.Outline then
                        segment.Outline.Thickness = 4 * val
                    end
                    if segment.Line then
...
local function getEspPlayerWeapon(player)
    ...
    local viewModels = game:GetService("Workspace"):FindFirstChild("ViewModels")
    ...
    for _, child in ipairs(viewModels:GetChildren()) do
        local weapon = weaponFromModelName(child.Name)
        if weapon then
            return weapon
        end
    end
...
local gun = pcall(function()
    return require(player.PlayerScripts.Modules.ItemTypes.Gun)
end) and require(player.PlayerScripts.Modules.ItemTypes.Gun) or nil
local util = modules.utility
...
manipulation.get_closest = function()
    ...
    for _, v in ipairs(players:GetPlayers()) do
        ...
        local head = v.Character:FindFirstChild("HitboxHead") or v.Character:FindFirstChild("Head")
        if not head then continue end

        local mag = (root.Position - head.Position).Magnitude
        if mag < dist then
            dist = mag
            target = head
            char = v.Character
...
manipulation.calculate_point = function(camcf, pos, char)
    if not config.target or not config.target.autoshoot then
        return camcf.Position, nil
    end
    ...
    local dir = campos - pos
    if dir.Magnitude <= 5 then
        return pos - dir.Unit * 2, nil
    end

    if workspace:Raycast(campos, pos - campos, params) == nil then
        return campos, nil
    end

    for height = 20, 120, 10 do
        local adjusted = campos + Vector3.new(0, height * 15, 0)
        if workspace:Raycast(pos, adjusted - pos, params) == nil then
            return campos, height + 8
        end
    end
    ...
    for i = 1, 8 do
        ...
        for j = 1, 8 do
            pitch = pitch + 10
            local manipulated = campos + (camcf * CFrame.fromOrientation(math.rad(yaw), math.rad(pitch), 0)).LookVector * 45
            if workspace:Raycast(manipulated, pos - manipulated, params) == nil then
...
if gun and util then
    local old = gun.StartShooting
    gun.StartShooting = function(...)
        local args = {...}
        local ret = {old(...)}

        if config.target and config.target.enabled and config.target.autoshoot and ret[3] then
            local target_part, target_char = manipulation.get_closest()
            if target_part and target_char then
                local cam = workspace.CurrentCamera and workspace.CurrentCamera.CFrame
                if cam then
                    local manip, height = manipulation.calculate_point(cam, target_part.Position, target_char)
                    local shoot_pos = (height == nil and manip) or cam.Position

                    ret[3] = {
                        [utf8.char(0)] = util:EncodeCFrame(
                            CFrame.new(shoot_pos.X, shoot_pos.Y + (height or 0), shoot_pos.Z) *
                            CFrame.Angles(CFrame.lookAt(shoot_pos, target_part.Position):ToOrientation())
                        ),
                        [utf8.char(1)] = height and util:EncodeCFrame(
                            CFrame.new(target_part.Position) *
                            CFrame.Angles(CFrame.lookAt(shoot_pos, target_part.Position):ToOrientation())
                        ) or util:EncodeCFrame(
                            CFrame.new(shoot_pos.X, shoot_pos.Y + (height or 0), shoot_pos.Z) *
                            CFrame.Angles(CFrame.lookAt(shoot_pos, target_part.Position):ToOrientation())
                        ),
...
local function instanceIsPlayerInLiveFolder(targetPlr)
...
local function instanceGetPlayerMatchId(targetPlr)
...
local function instanceIsInActiveMatch()
    local lp = player
    ...
    if instanceIsPlayerInLiveFolder(lp) then
        return true
    end
    ...
    local myMatchId = instanceGetPlayerMatchId(lp)
    if myMatchId then
        for _, plr in players:GetPlayers() do
            if plr ~= lp and instanceGetPlayerMatchId(plr) == myMatchId then
                return true
            end
        end
    end
...
local function instanceRefreshLobbyCache()
    local now = os.clock()
    if now - _instanceLobbyCache.t < 0.15 then
        return _instanceLobbyCache.v
    end
    _instanceLobbyCache.t = now
    _instanceLobbyCache.v = not instanceIsInActiveMatch()
    return _instanceLobbyCache.v
end

getgenv().InstanceIsInActiveMatch = instanceIsInActiveMatch
getgenv().InstanceIsInLobby = instanceRefreshLobbyCache
...
local indtext1 = Drawing.new("Text")
indtext1.Size = 13
indtext1.Font = 2
indtext1.Outline = true
indtext1.Text = ""
indtext1.Color = Color3.new(1, 1, 1)
indtext1.Visible = false
indtext1.Center = true

local indtext2 = Drawing.new("Text")
indtext2.Size = 13
indtext2.Font = 2
indtext2.Outline = true
indtext2.Text = ""
indtext2.Color = Color3.fromRGB(0, 186, 255)
indtext2.Visible = false
indtext2.Center = true
...
local ammoStatusCache = {item = nil, current = 0, maxAmmo = 0, melee = false, updatedAt = 0}

local function getammo(forceRefresh)
    local now = tick()
    local fighter = localfighter or local_fighter or localFighter
    local item = fighter and fighter.EquippedItem

    if not forceRefresh
        and item == ammoStatusCache.item
        and ammoStatusCache.updatedAt > 0
        and now - ammoStatusCache.updatedAt < 0.15 then
        return ammoStatusCache.current, ammoStatusCache.maxAmmo, ammoStatusCache.melee
    end

    if item then
        local current = item:Get("CurrentAmmo") or item:Get("Ammo") or 0
        local maxAmmo = item:Get("MaxAmmo") or item:Get("MaxBullets") or current
        local isMelee = false
        pcall(function()
            local name = item:Get("Name") or item.Name
            if type(name) == "string" and name:lower():find("melee", 1, true) then
                isMelee = true
            end
        end)

        ammoStatusCache.item = item
        ammoStatusCache.current = tonumber(current) or 0
        ammoStatusCache.maxAmmo = tonumber(maxAmmo) or 0
        ammoStatusCache.melee = isMelee
        ammoStatusCache.updatedAt = now
        return ammoStatusCache.current, ammoStatusCache.maxAmmo, ammoStatusCache.melee
    end

    ammoStatusCache.item = nil
    ammoStatusCache.current = 0
    ammoStatusCache.maxAmmo = 0
    ammoStatusCache.melee = false
    ammoStatusCache.updatedAt = now
    return 0, 0, false
end

local function isItemReloading()
...
local function updateRageAmmoState()
    local current, maxAmmo, melee = getammo()
    config.state.outofammo = not melee and current <= 0
    local reloading = isItemReloading()
...
local function applyRageReloadHide(_checksling)
    if not config.ragestatus.hideOnReload then
        return false
    end
    if not config.state.reloading then
        return false
    end
    return true
end
...
local function clampVoidSpamTime(v)
    return math.clamp(tonumber(v) or 1, 0.1, 2)
end

local function randomVoidSpamDuration(minT, maxT)
    local a = clampVoidSpamTime(minT)
    local b = clampVoidSpamTime(maxT)
    if b < a then
        a, b = b, a
    end
    if a == b then
...
-- Void For Real (void_for_real.txt) — void spam hide + reload hide
pcall(function()
    runsvc:UnbindFromRenderStep("InstanceVoidSpamForReal")
end)
...
local function clearVoidForRealClient()
    _G.clientCF = nil
    _G.clientLV = nil
    _G.clientAV = nil
end

local function isVoidForRealActive()
    if config.state.reloading and config.ragestatus.hideOnReload ~= false then
        return true
    end
    local vs = config.voidspam
    return vs.enabled == true and vs.phase == "hide"
end

runsvc:BindToRenderStep("VoidForReal_csync", Enum.RenderPriority.First.Value, function()
    if not isVoidForRealActive() or not _G.clientCF then
        return
    end
    local char = player.Character
    ...
    if hrp then
        pcall(function()
            hrp.CFrame = _G.clientCF
            hrp.AssemblyLinearVelocity = _G.clientLV
            hrp.AssemblyAngularVelocity = _G.clientAV
        end)
    end
end)

local voidForRealHeartbeatConn = runsvc.Heartbeat:Connect(function()
    updateRageAmmoState()
    if not isVoidForRealActive() then
        return
    end
    local char = player.Character
    local hrp = char and char:FindFirstChild("HumanoidRootPart")
    ...
    _G.clientCF = hrp.CFrame
    _G.clientLV = hrp.AssemblyLinearVelocity
    _G.clientAV = hrp.AssemblyAngularVelocity
    pcall(function()
        local lim = 100000
        hrp.CFrame = CFrame.new(
            betterRandom(-lim, lim, -50000, 50000),
            betterRandom(-lim, lim, -50000, 50000),
            betterRandom(-lim, lim, -50000, 50000)
...
local function getAutoTargetAttackSettings()
...
local function applyAutoTargetOrbitHeight(weapon)
    local settings = getAutoTargetAttackSettings()
    config.orbit.height = settings.height
end

local function getAutoTargetBasePos(root)
    if not root then
        return Vector3.zero
    end
    local settings = getAutoTargetAttackSettings()
...
local function refreshAutoTargetAttackMotion()
    applyAutoTargetOrbitHeight(getweapon())
    updateorbitrad()
end

local function updateorbitrad()
    local settings = getAutoTargetAttackSettings()
...
local function stopsync()
    ...
    if underground_enabled and underground_oldpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart then
        pcall(function()
            localfighter.Entity.RootPart.CFrame = underground_oldpos
        end)
    else
        
        if config.orbit.savedpos and localfighter and localfighter.Entity and localfighter.Entity.RootPart then
            local ok, pos = pcall(function() return config.orbit.savedpos end)
            if ok and pos then
                local y = pos.Position and pos.Position.Y or (pos.Y or 0)
                
                if y > -10000 and y < 10000 then
                    pcall(function()
                        localfighter.Entity.RootPart.CFrame = config.orbit.savedpos
                    end)
                end
            end
        end
...
local function startsync()
    ...
    local weapon = getweapon()
    applyAutoTargetOrbitHeight(weapon)
    updateorbitrad()

    
    if localfighter and localfighter.Entity and localfighter.Entity.RootPart then
        
        if not ((config.voidspam and config.voidspam.phase == "hide") or underground_enabled) then
    ...
    config.orbit.connection = runsvc.Heartbeat:Connect(function(dt)
        if vhState and vhState.active then return end
        if not config.state.csyncactive then return end
        ...
        local checksling = false
        if _weapon then
            checksling = _weapon:lower():find("slingshot") ~= nil
        end
        ...
        if config.voidspam.enabled then
            ...
            if elapsed >= config.voidspam.currentduration then
                ...
                if config.voidspam.phase == "shoot" then
                    ...
                    config.voidspam.currentduration = randomVoidSpamDuration(config.voidspam.hide_min, config.voidspam.hide_max)
                else
                    config.voidspam.phase = "shoot"
                    
                    config.voidspam.currentduration = randomVoidSpamDuration(config.voidspam.shoot_min, config.voidspam.shoot_max)
                end
        ...
        if applyRageReloadHide(checksling) then
            return
        end
        ...
        if not checksling and localfighter and localfighter.Entity and localfighter.Entity.RootPart then
            local dist = (root.Position - localfighter.Entity.RootPart.Position).Magnitude
            if dist > 1000 then
                stopsync()
                return
            end
        ...
        local targetpos = getAutoTargetBasePos(root)
        ...
        if not underground_enabled and not config.state.outofammo and not config.state.reloading then
...
local function autoshoot()
    ...
    local weapon = getweapon()
    if weapon then
        checksling = weapon:lower():find("slingshot") ~= nil
    end

    if vhState and vhState.active and checksling then
        local root = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
        shootPos = root and root.Position or Vector3.new()
        targetPos = shootPos + Vector3.new(0, 1000, 0)
    else
        if vhState and vhState.active then
            local targetRoot = targetChar:FindFirstChild("HumanoidRootPart")
            shootPos = targetRoot and (targetRoot.Position + Vector3.new(0, 18, 0)) or Vector3.new()
...
local function drawind()
    if not config.visualizer.enabled or not config.target.enabled or not config.target.character then
        tracerline.Visible = false
        traceroutline.Visible = false
        indtext1.Visible = false
        indtext2.Visible = false
    ...
    local wts = getgenv().InstanceWorldToScreen or worldToScreen
    local vp, visible = wts(root.Position, camera)
    if not visible or not vp then
        tracerline.Visible = false
        traceroutline.Visible = false
        indtext1.Visible = false
        indtext2.Visible = false
    ...
    local pos3 = userinput:GetMouseLocation()
    if config.visualizer.tracer.start_point == "muzzle" then
        local muzzlePos = muzzlepos()
        if muzzlePos then
            local muzzleScreen, mOk = wts(muzzlePos, camera)
            if mOk and muzzleScreen then
                pos3 = Vector2.new(muzzleScreen.X, muzzleScreen.Y)
            end
        end
    end

    if config.visualizer.tracer.outline then
        traceroutline.From = pos3
        traceroutline.To = Vector2.new(vp.X, vp.Y)
        traceroutline.Thickness = config.visualizer.tracer.outline_thickness
        traceroutline.Color = config.visualizer.tracer.outline_color
        traceroutline.Visible = true
    else
        traceroutline.Visible = false
    end

    tracerline.From = pos3
    tracerline.To = Vector2.new(vp.X, vp.Y)
    tracerline.Thickness = config.visualizer.tracer.thickness
    tracerline.Color = config.visualizer.tracer.color
    tracerline.Visible = true

    local targetPlayer = players:GetPlayerFromCharacter(config.target.character)
    local parts = {}
    for _, option in pairs(config.visualizer.indicator.display_options) do
        ...
        elseif option == "hit reg" and targetPlayer then
            local lastTime = lastdamagetime[targetPlayer] or 0
            table.insert(parts, string.format("%.1fs", tick() - lastTime))
        end
    end
    ...
    if #parts > 0 then
        ...
        indtext1.Text = fullText
        indtext1.Color = config.visualizer.indicator.color
        indtext1.Position = Vector2.new(screenCenter.X, screenCenter.Y - 15)
        indtext1.Visible = true

        indtext2.Text = fullText
        indtext2.Color = config.visualizer.indicator.accent_color
        indtext2.Position = Vector2.new(screenCenter.X, screenCenter.Y + 15)
        indtext2.Visible = true
    else
        indtext1.Visible = false
        indtext2.Visible = false
    end
...
local function checksling()
    local weapon = getweapon()
    return weapon and weapon:lower():find("slingshot") ~= nil
end
...
local function voidhide()
    ...
    vhState.hrp = player.Character and player.Character:FindFirstChild("HumanoidRootPart")
    if not vhState.hrp then return end

    vhState.mainConnection = runsvc.Heartbeat:Connect(function()
        if not vhState.active then return end

        if not config.target.character then return end

        local trg = config.target.character:FindFirstChild("HumanoidRootPart")

        if trg and vhState.hrp then
            local flat = Vector3.new(trg.CFrame.LookVector.X, 0, trg.CFrame.LookVector.Z).Unit
            local newpos = trg.Position + Vector3.new(0, 15, 0) + flat * 5
            vhState.hrp.CFrame = CFrame.new(newpos) * vhState.hrp.CFrame.Rotation
            vhState.hrp.AssemblyLinearVelocity = Vector3.zero
            vhState.currentVoidPos = vhState.hrp.Position
        else
            stopvoid()
        end
    end)
...
local function updatesling()
    ...
    if slingon then
        ...
        if config.voidhide.enabled and hastarget then
            voidhide()
        else
            stopvoid()
        end
...
runsvc.RenderStepped:Connect(function()
    ...
    drawind()
    if config.ragestatus and config.ragestatus.enabled and ragebot.updateRageStatus then
        ragebot.updateRageStatus()
...
local function createRageStatusLabel()
...
local rageStatusLine1 = createRageStatusLabel()
local rageStatusLine2 = createRageStatusLabel()

local function setRageStatusLabelStyle(label)
    label.Size = config.ragestatus.textSize or 13
    label.Font = 3
    label.Outline = true
    label.Color = config.ragestatus.color
...
local function getRageStatusAnchor()
...
local function isRageInVoid()
    if vhState.active then
        return true
    end
    if config.voidspam.enabled and config.voidspam.phase == "hide" then
        return true
    end
    if config.state.reloading and config.ragestatus.hideOnReload ~= false then
        return true
...
local function isRageKilling()
    if not isRageCombatActive() then
        return false
    end
    if config.state.reloading or isRageInVoid() then
        return false
    end
    if config.target.immune then
        return false
...
local function getRageHudAmmoText()
    local cur, maxAmmo, melee = getammo()
    if melee then
        return "melee"
    end
    return string.format("%d/%d", math.floor(cur), math.max(math.floor(maxAmmo), 1))
end

local function formatVoidSpamDurationRange(minT, maxT)
    local a = clampVoidSpamTime(minT)
    local b = clampVoidSpamTime(maxT)
    if b < a then
        a, b = b, a
...
local function getVoidSpamConfigDetail()
    ...
    return string.format(
        "void %s atk · %s hide",
        formatVoidSpamDurationRange(vs.shoot_min, vs.shoot_max),
        formatVoidSpamDurationRange(vs.hide_min, vs.hide_max)
    )
end

local function getRageStatusLines()
    local now = tick()

    if isRageKilling() then
    ...
    if isRageInVoid() then
        return "ragebot: voiding", ""
    end
    if isRageKilling() then
        local name = getRageKillTargetName()
        ...
        local voidDetail = getVoidSpamConfigDetail()
        if voidDetail ~= "" then
    ...
    if isRageCombatActive() and config.target.enabled and config.target.character then
...
local function updateRageStatus()
    if not config.ragestatus.enabled then
        rageStatusLine1.Visible = false
        rageStatusLine2.Visible = false
        return
    end

    local anchor = getRageStatusAnchor()
    ...
    if ragePerf.lastRageColor ~= config.ragestatus.color then
        ...
        setRageStatusLabelStyle(rageStatusLine1)
        setRageStatusLabelStyle(rageStatusLine2)
    end

    local mainLine, detailLine = getRageStatusLines()
    local ammoLine = showAmmo and getRageHudAmmoText() or ""
...
ragebot.updateRageStatus = updateRageStatus
ragebot.hideRageStatus = function()
    rageStatusLine1.Visible = false
    rageStatusLine2.Visible = false
end
ragebot.clearVoidForRealClient = clearVoidForRealClient
...
local function syncAttackPositionDeps()
    local mode = config.target.attackPosition or "default"
...
local attackPosDropdown = rageui.ragebottab:AddDropdown("AutoTargetAttackPos", {
    ...
    Callback = function(val)
        ...
        syncAttackPositionDeps()
        if config.state.csyncactive then
            refreshAutoTargetAttackMotion()
        end
    end,
})
...
attackUnderDep:AddSlider("UnderAttackOffset", {
    ...
    Callback = function(val)
        ...
        if config.target.attackPosition == "under"
            ...
            and config.state.csyncactive then
            refreshAutoTargetAttackMotion()
        end
    end,
})
...
local function onCustomAttackSettingChanged()
    if config.target.attackCustomEnabled and config.state.csyncactive then
        refreshAutoTargetAttackMotion()
    end
end
...
attackCustomDep:AddSlider("CustomAttackFront", {
    ...
    Rounding = 1,
    Compact = true,
    Tooltip = "studs in front of target (their look direction)",
    Callback = function(val)
...
attackCustomDep:AddSlider("CustomAttackSide", {
    ...
    Rounding = 1,
    Compact = true,
    Tooltip = "studs to the target's right",
    Callback = function(val)
...
attackCustomDep:AddSlider("CustomAttackVertical", {
    ...
    Rounding = 1,
    Compact = true,
    Tooltip = "world Y offset from target root",
    Callback = function(val)
...
syncAttackPositionDeps()
task.defer(syncAttackPositionDeps)

rageui.ragebottab:AddToggle("AutoShootT", {
    Text = "manipulation",
    Default = false,
    Callback = function(val) config.target.autoshoot = val end
})
...
rageui.ragebottab:AddToggle("VoidSpam", {
    ...
    Callback = function(val)
        ...
        if val then
            ...
            config.voidspam.phase = nil
            if ragebot.clearVoidForRealClient then
                ragebot.clearVoidForRealClient()
            end
        end
    end
})

rageui.ragebottab:AddDropdown("VoidSpamMode", {
    Text = "voidspam mode",
    Default = "default",
    Values = { "default", "cloudy void" },
    Callback = function(val)
        config.voidspam.mode = val
        if ragebot.clearVoidForRealClient then
            ragebot.clearVoidForRealClient()
        end
    end,
...
rageui.ragebottab:AddToggle("VoidHideT", {
    Text = "voidhide",
    Default = false,
    Callback = function(val)
        ...
        if not val then
            stopvoid()
            if config.target.enabled and config.target.character and valid(config.target.character) and not config.target.immune then
                startsync()
            end
        end
    end
...
local rageStatusToggle = rageui.ragebotvisualtab:AddToggle("RageStatus", {
    ...
    Callback = function(val)
        ...
        if not val and ragebot.hideRageStatus then
            ragebot.hideRageStatus()
        end
    end,
...
rageui.ragebotvisualtab:AddToggle("VisIndicatorEnabled", {
    Text = "indicator",
    Default = false,
    Callback = function(val) config.visualizer.indicator.enabled = val end
}):AddColorPicker("VisIndicatorColor", {
    Default = config.visualizer.indicator.color,
    Callback = function(val) config.visualizer.indicator.color = val end
}):AddColorPicker("VisIndicatorAccent", {
    Default = config.visualizer.indicator.accent_color,
    Callback = function(val) config.visualizer.indicator.accent_color = val end
})

rageui.ragebotvisualtab:AddDropdown("VisIndOptions", {
    Text = "display",
    Default = {"name", "position", "hit reg"},
    Values = {"name", "position", "hit reg"},
    Multi = true,
    Callback = function(val) config.visualizer.indicator.display_options = val end
})
end
end
...
local tpGroup = Tabs.Character:AddLeftGroupbox("third person")
...
if not config.profile.thirdperson then
    config.profile.thirdperson = { enabled = false, x = 5, y = 4, z = 15 }
end

local thirdpersonOffset = Vector3.new(5, 4, 15)
local thirdpersonCam = nil
local thirdpersonConn = nil
local thirdpersonRenderConn = nil
local thirdpersonPreviousPublicState = nil
local thirdpersonPreviousPovState = nil
local thirdpersonPreviousActive = nil
local thirdpersonPreviousOverride = nil

local function applyThirdPersonOffset()
    if not thirdpersonCam then
        return
    end

    local cs = thirdpersonCam.CameraState
    if not cs or not cs.States or not cs.States.ThirdPerson or not thirdpersonCam._pov_spring then
        return
    end

    pcall(function()
        thirdpersonCam._third_person_override = true
        cs._public_state = cs.States.ThirdPerson
        cs._pov_state = cs.States.ThirdPerson
        cs._is_active = true
        thirdpersonCam._pov_spring.Target = thirdpersonOffset
    end)
end

local function thirdpersonShowChar()
    local char = LocalPlayer.Character
    if not char then return end
...
local function thirdpersonDisconnect()
    ...
    if thirdpersonCam and thirdpersonCam.CameraState then
        pcall(function()
            local cs = thirdpersonCam.CameraState
            if thirdpersonCam._pov_spring then
                thirdpersonCam._pov_spring.Target = Vector3.zero
            end
            local restored = false
            if cs.States and cs.States.FirstPerson then
                cs._public_state = cs.States.FirstPerson
                cs._pov_state = cs.States.FirstPerson
                cs._is_active = false
                cs.StateChanged:Fire(cs.States.FirstPerson, cs.States.ThirdPerson)
                restored = true
            end
            if (not restored) and thirdpersonPreviousPublicState ~= nil then
                cs._public_state = thirdpersonPreviousPublicState
            end
            if (not restored) and thirdpersonPreviousPovState ~= nil then
                cs._pov_state = thirdpersonPreviousPovState
            end
            if (not restored) and thirdpersonPreviousActive ~= nil then
                cs._is_active = thirdpersonPreviousActive
            end
            if restored then
                thirdpersonCam._third_person_override = false
            elseif thirdpersonPreviousOverride ~= nil then
                thirdpersonCam._third_person_override = thirdpersonPreviousOverride
            else
                thirdpersonCam._third_person_override = false
            end
    ...
    thirdpersonCam = nil
    thirdpersonPreviousPublicState = nil
    thirdpersonPreviousPovState = nil
    thirdpersonPreviousActive = nil
    thirdpersonPreviousOverride = nil
end

local function enableThirdPerson()
    thirdpersonDisconnect()
    thirdpersonOffset = Vector3.new(config.profile.thirdperson.x, config.profile.thirdperson.y, config.profile.thirdperson.z)
    thirdpersonConn = RunService.Heartbeat:Connect(function()
        for _, v in pairs(getgc(true)) do
            if type(v) == "table" and rawget(v, "_base_fov") ~= nil and rawget(v, "_pov_spring") ~= nil and rawget(v, "CameraState") ~= nil then
                thirdpersonCam = v
                break
            end
        end
        if thirdpersonCam and thirdpersonCam.CameraState and thirdpersonCam._pov_spring then
            if thirdpersonConn then
                thirdpersonConn:Disconnect()
                thirdpersonConn = nil
            end
            local cs = thirdpersonCam.CameraState
            thirdpersonPreviousPublicState = cs._public_state
            thirdpersonPreviousPovState = cs._pov_state
            thirdpersonPreviousActive = cs._is_active
            thirdpersonPreviousOverride = thirdpersonCam._third_person_override
            applyThirdPersonOffset()
            pcall(function()
                cs.StateChanged:Fire(cs.States.ThirdPerson, thirdpersonPreviousPublicState)
            end)
            thirdpersonShowChar()
            thirdpersonRenderConn = RunService.RenderStepped:Connect(function()
                if not thirdpersonCam or not thirdpersonCam.CameraState or not thirdpersonCam._pov_spring then
                    return
                end
                local currentState = thirdpersonCam.CameraState
                thirdpersonCam._third_person_override = true
                if currentState._public_state ~= currentState.States.ThirdPerson then
                    local prev = currentState._public_state
                    applyThirdPersonOffset()
                    pcall(function()
                        currentState.StateChanged:Fire(currentState.States.ThirdPerson, prev)
                    end)
                    thirdpersonShowChar()
                else
                    applyThirdPersonOffset()
...
local function updatethirdperson()
    if not config.profile.thirdperson.enabled then
        thirdpersonDisconnect()
        return
    end
...
tpGroup:AddToggle("ThirdPersonEnabled", {
    Text = "enable",
    Default = false,
    Callback = function(val)
        config.profile.thirdperson.enabled = val
        updatethirdperson()
    end
})

tpGroup:AddDualSlider('OffsetX', 'OffsetY', {
    Text = 'x',
    Default = 5,
    Min = -10,
    Max = 10,
    ...
    Compact = true,
    Callback = function(v) 
        config.profile.thirdperson.x = v
        if config.profile.thirdperson.enabled then
            thirdpersonOffset = Vector3.new(config.profile.thirdperson.x, config.profile.thirdperson.y, config.profile.thirdperson.z)
            applyThirdPersonOffset()
        end
    end
    ...
    Compact = true,
    Callback = function(v) 
        config.profile.thirdperson.y = v
        if config.profile.thirdperson.enabled then
            thirdpersonOffset = Vector3.new(config.profile.thirdperson.x, config.profile.thirdperson.y, config.profile.thirdperson.z)
            applyThirdPersonOffset()
        end
...
tpGroup:AddSlider("OffsetZ", {
    ...
    Compact = true, 
    Callback = function(v) 
        config.profile.thirdperson.z = v
        if config.profile.thirdperson.enabled then
            thirdpersonOffset = Vector3.new(config.profile.thirdperson.x, config.profile.thirdperson.y, config.profile.thirdperson.z)
            applyThirdPersonOffset()
        end
...
local SlfMtrl = {
    ...
    color2 = Color3.fromRGB(80, 180, 255),
    color3 = Color3.fromRGB(180, 120, 255),
    materialName = "Neon",
    transparency = 0.1,
    pulseSpeed = 3,
    phase = 0,
...
local function curweap2()
    return getCachedWeaponName()
end

local fighter_controller = require(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.FighterController)
local local_fighter = fighter_controller.LocalFighter
local underground_enabled = false
local camera_controller = require(game:GetService("Players").LocalPlayer.PlayerScripts.Controllers.CameraController)
...
antiaimbox:AddSlider("customangle", {
    Text = "custom angle",
    Default = 0,
    Min = 0,
    Max = 360,
...
antiaimbox:AddToggle("underground", {
    Text = "underground",
    Default = false,
    Callback = function(value)
...
local function applyHumBreakerMode(dt, hrp, humChar)
    local mode = hmndbreaker.Mode or "vertical"
    local spin = hmndbreaker.SpinSpeed or 0

    if mode == "vertical" then
        if hmndbreaker.BounceIntensity > 0 then
            hmndbreaker.BouncePhase = hmndbreaker.BouncePhase + dt * 8
            humChar.HipHeight = math.sin(hmndbreaker.BouncePhase) * hmndbreaker.BounceIntensity * 0.15
        elseif humChar.HipHeight ~= 0 then
            humChar.HipHeight = 0
        end
        if spin > 0 then
            hrp.CFrame = hrp.CFrame * CFrame.Angles(math.rad(spin) * dt * 0.35, 0, 0)
        end
    elseif mode == "horizontal" then
        humChar.HipHeight = 0
        hrp.CFrame = hrp.CFrame * CFrame.Angles(
            math.rad(spin) * dt,
            math.rad(spin * 0.5) * dt,
            math.rad(spin * 0.75) * dt
        )
    elseif mode == "solar system" then
        humChar.HipHeight = math.sin(tick() * 6) * 0.35
        hmndbreaker.SolarAngle = hmndbreaker.SolarAngle + dt * math.max(spin / 45, 0.5)
        local radius = hmndbreaker.SolarRadius or 5
        local orbitOffset = Vector3.new(
            math.cos(hmndbreaker.SolarAngle) * radius * 0.12,
            math.sin(hmndbreaker.SolarAngle * 2) * 0.08,
            math.sin(hmndbreaker.SolarAngle) * radius * 0.12
        )
        hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(spin) * dt, 0) * CFrame.new(orbitOffset)
    elseif mode == "no head" then
        humChar.HipHeight = 0
        hideCharacterHead(LocalPlayer.Character)
        local head = LocalPlayer.Character and LocalPlayer.Character:FindFirstChild("Head")
        local hrpPos = hrp.Position
        if head then
            head.CFrame = CFrame.new(hrpPos + Vector3.new(0, -12, 0))
        end
        if spin > 0 then
            hrp.CFrame = hrp.CFrame * CFrame.Angles(0, math.rad(spin) * dt, 0)
        end
    end
end

local function cleanuphumbreaker()
    if hmndbreaker.Track then 
        hmndbreaker.Track:Stop() 
        hmndbreaker.Track:Destroy() 
        hmndbreaker.Track = nil 
    end
    if hmndbreaker.Heartbeat then 
        hmndbreaker.Heartbeat:Disconnect() 
        hmndbreaker.Heartbeat = nil 
    end
    if hmndbreaker.AnimConnection then 
        hmndbreaker.AnimConnection:Disconnect() 
        hmndbreaker.AnimConnection = nil 
    end
    hmndbreaker.BouncePhase = 0
    hmndbreaker.SolarAngle = 0
    local char = LocalPlayer.Character
    restoreHiddenHead(char)
    local hum = char and char:FindFirstChildOfClass("Humanoid")
    if hum then hum.HipHeight = 0 end
end

local function runhumbreaker()
    if not hmndbreaker.Active then return end
    local hum = hum2()
    if not hum then return end
    cleanuphumbreaker()
    
    local anim = Instance.new("Animation")
    anim.AnimationId = hmndbreaker.EmoteId
    hmndbreaker.Track = hum:LoadAnimation(anim)
    hmndbreaker.Track:Play(0, 1, 1)
    
    hmndbreaker.Heartbeat = RunService.Heartbeat:Connect(function(dt)
        if hmndbreaker.Track and hmndbreaker.Active then
            hmndbreaker.Track.TimePosition = hmndbreaker.FreezeTime
            hmndbreaker.Track:AdjustSpeed(0)
        end

        local char = LocalPlayer.Character
        local hrp = char and char:FindFirstChild("HumanoidRootPart")
        local humChar = char and char:FindFirstChildOfClass("Humanoid")
        if not hrp or not humChar then return end

        applyHumBreakerMode(dt, hrp, humChar)
    end)
    
    hmndbreaker.AnimConnection = hum.AnimationPlayed:Connect(function(newtrack)
        if hmndbreaker.Active and hmndbreaker.Track and newtrack ~= hmndbreaker.Track then
            local delay = hmndbreaker.Gap + math.random() * hmndbreaker.Gap
            task.delay(delay, runhumbreaker)
        end
    end)
end

local function togglehumbreaker(state)
    hmndbreaker.Active = state
    if state then
        runhumbreaker()
    else
        cleanuphumbreaker()
...
LocalPlayer.CharacterAdded:Connect(function()
    task.wait(0.25)
    if hmndbreaker.Active then 
        runhumbreaker() 
    end
end)

local humBreakerToggle = v555:AddToggle('hmndbreaker', {
    Text = 'humanoid breaker',
    Default = false,
    Callback = function(Value)
        togglehumbreaker(Value)
    end
})

local humBreakerDepBox = v555:AddDependencyBox()

humBreakerDepBox:AddDropdown('hmndbreaker_mode', {
    Text = 'mode',
    Default = hmndbreaker.Mode,
    Values = {'vertical', 'horizontal', 'solar system', 'no head'},
    Callback = function(value)
        local char = LocalPlayer.Character
        restoreHiddenHead(char)
        hmndbreaker.Mode = value
        hmndbreaker.BouncePhase = 0
        hmndbreaker.SolarAngle = 0
        local hum = char and char:FindFirstChildOfClass("Humanoid")
        if hum then hum.HipHeight = 0 end
...
humBreakerDepBox:AddSlider('hmndbreaker_spin', {
    Text = 'spin speed',
    Default = 0,
    Min = 0,
    Max = 720,
    ...
    Suffix = '°/s',
    Callback = function(value)
        hmndbreaker.SpinSpeed = value
    end
})

humBreakerDepBox:AddSlider('hmndbreaker_gap', {
    Text = 'gap',
    Default = 0.025,
    Min = 0,
    Max = 0.5,
    ...
    Suffix = 's',
    Callback = function(value)
        hmndbreaker.Gap = value
    end
})

humBreakerDepBox:AddSlider('hmndbreaker_bounce', {
    ...
    Compact = true,
    Callback = function(value)
        hmndbreaker.BounceIntensity = value
        if value == 0 then
            hmndbreaker.BouncePhase = 0
            local char = LocalPlayer.Character
            local hum = char and char:FindFirstChildOfClass("Humanoid")
            if hum and hmndbreaker.Mode == "vertical" then hum.HipHeight = 0 end
        end
    end
})

humBreakerDepBox:AddSlider('hmndbreaker_solar_radius', {
    Text = 'solar radius',
    Default = hmndbreaker.SolarRadius,
    Min = 1,
    Max = 20,
    Rounding = 1,
    Compact = true,
    Callback = function(value)
        hmndbreaker.SolarRadius = value
    end
})

humBreakerDepBox:AddSlider('hmndbreaker_freeze', {
    Text = 'freeze time',
    Default = 0.1265,
    Min = 0,
    Max = 2,
    ...
    Suffix = 's',
    Callback = function(value)
        hmndbreaker.FreezeTime = value
    end
})

humBreakerDepBox:SetupDependencies({
    { humBreakerToggle, true }
})
...
v555:AddToggle("AntiFlashbang", {
    ...
    Callback = function(val)
        if not val then
            if _G.antiFlashCon then
                _G.antiFlashCon:Disconnect()
                _G.antiFlashCon = nil
            end
            return
        end
        ...
        _G.antiFlashCon = game:GetService("RunService").RenderStepped:Connect(function()
            local sc = require(plr.PlayerScripts.Controllers.SpectateController)
            local subj = sc.CurrentSubject
            if subj and subj.FighterInterface then
                if old then
                    restorefunction(old)
                end
                local fl = subj.FighterInterface.Flashed
                if fl and fl.AttemptToFlash then
                    hookfunction(fl.AttemptToFlash, function() return end)
                    getgenv().old = fl.AttemptToFlash
                end
            end
...
getgenv().InstanceCosmeticUIState = getgenv().InstanceCosmeticUIState or {
    gui = nil,
    frame = nil,
    inner = nil,
    panel = nil,
...
v555:AddToggle("AnySkinMode", {
    Text = "any model",
    Default = getgenv().InstanceCosmeticUIState.anyModel or false,
    Tooltip = "allow cosmetic changer to use any gun with any skin",
    Callback = function(val)
        getgenv().InstanceCosmeticUIState.anyModel = val and true or false
...
local function openCosmeticChanger(showUi)
    ...
    local SKINS_FOLDER = "instance skins"
    ...
    local function saveSkins()
        ...
        task.delay(0.15, function()
            ...
            for weapon, types in pairs(equipped) do
                data.equipped[weapon] = {}
                for ctype, cdata in pairs(types) do
            ...
            for weapon, favs in pairs(favorites) do
                ...
                for cname, isFav in pairs(favs) do
    ...
    local function loadSkins()
        ...
        if type(data.equipped) == "table" then
            for weapon, types in pairs(data.equipped) do
                if type(types) == "table" then
                    equipped[weapon] = equipped[weapon] or {}
                    for ctype, ref in pairs(types) do
        ...
        if type(data.favorites) == "table" then
            for weapon, favs in pairs(data.favorites) do
    ...
    local old = data_ctrl.Get; data_ctrl.Get = function(self, data_key)
        ...
        if data_key == "CosmeticInventory" then
            ...
            for cosmetic_name, cosmetic_data in pairs(cosmetic_lib.Cosmetics) do
        ...
        if data_key == "FavoritedCosmetics" then
            ...
            for weapon_name, weapon_favs in pairs(favorites) do
                fav[weapon_name] = fav[weapon_name] or {}
                for cosmetic_name, is_fav in pairs(weapon_favs) do
    ...
    local resolveEquippedWeaponName
    local old = data_ctrl.GetWeaponData; data_ctrl.GetWeaponData = function(self, weapon_name)
        ...
        if enabled then
            ...
            if equipped_name and equipped[equipped_name] then
                for cosmetic_type, cosmetic_data in pairs(equipped[equipped_name]) do
    ...
    old_nc = hookmetamethod(game, "__namecall", function(self, ...)
        ...
        if method == "FireServer" and enabled then
            if self == equip_remote then
                local weapon = args[1]
                local cosmetic_type = args[2]
                local cosmetic_name = args[3]
                local extra_opts = args[4] or {}

                equipped[weapon] = equipped[weapon] or {}

                if not cosmetic_name or cosmetic_name == "None" or cosmetic_name == "" then
                    equipped[weapon][cosmetic_type] = nil
                    if not next(equipped[weapon]) then
                        equipped[weapon] = nil
                    end
                else
                    local cosmetic_data = clone_cosmetic(cosmetic_name, cosmetic_type, {
                        inverted = extra_opts.IsInverted
                    })
                    if cosmetic_data then
                        equipped[weapon][cosmetic_type] = cosmetic_data
                    end
                end
    ...
    local uiState = getgenv().InstanceCosmeticUIState
    ...
    gui.Name = "instance_cosmetic_changer"
    ...
    outer.Size = UDim2.fromOffset(820, 540)
    outer.Position = UDim2.new(0.5, -410, 0.5, -270)
    ...
    cosmeticSearch.Size = UDim2.new(1, -8, 0, 22)
    ...
    cosmeticSearch.Font = Enum.Font.Code
    cosmeticSearch.TextSize = 14
    ...
    skinHint.Size = UDim2.fromOffset(168, 52)
    skinHint.Position = UDim2.fromOffset(12, 14)
    ...
    wrapToggleRow.Size = UDim2.fromOffset(176, 54)
    wrapToggleRow.Position = UDim2.fromOffset(188, 12)
    ...
    actionRow.AnchorPoint = Vector2.new(1, 0.5)
    actionRow.Size = UDim2.fromOffset(340, 32)
    actionRow.Position = UDim2.new(1, -12, 0.5, 0)
    ...
    applyAllBtn.Name = "ApplyAllBtn"
    applyAllBtn.Size = UDim2.fromOffset(148, 28)
    applyAllBtn.Text = "apply selected to all"
    applyAllBtn.Font = Enum.Font.Code
    applyAllBtn.TextSize = 12
    applyAllBtn.BorderSizePixel = 1
    applyAllBtn.AutoButtonColor = false
    applyAllBtn.Parent = actionRow
    ...
    local function getWeapons()
        ...
        for _, cdata in pairs(cosmetic_lib.Cosmetics or {}) do
            if type(cdata) == "table" and type(cdata.Weapons) == "table" then
                for _, wn in ipairs(cdata.Weapons) do
                    addWeapon(wn)
                end
            end
        end
    ...
    local function cosmeticNorm(s)
        if type(s) ~= "string" then return "" end
        return s:lower():gsub("[%s%-%_]+", "")
    end

    resolveEquippedWeaponName = function(weaponName, weapon_data)
        if not weaponName then return nil end
        if equipped[weaponName] then
            return weaponName
        end
        if weapon_data then
            for _, key in ipairs({weapon_data.Name, weapon_data.Weapon, weapon_data.WeaponName}) do
    ...
    for weaponName, skinList in pairs(weaponSkinSeedRaw) do
    ...
    local function getCosmeticsFor(weaponName, cosmeticType)
        ...
        local function matchesWeapon(cdata)
            ...
            if cosmeticType == "Wrap" and not hasRestriction then
                return true
            end
            return false
        end

        for cname, cdata in pairs(cosmetic_lib.Cosmetics or {}) do
    ...
    local function addTypeButton(typeName, order)
        ...
        btn.Position = UDim2.new(0, (order - 1) * 94, 0, 2)
    ...
    local function fireEquipCosmetic(weaponName, ctype, cname, extraOpts)
        ...
        equip_remote:FireServer(weaponName, ctype, cname, extraOpts)
    end
    ...
    local function styleThemedToggle(toggleData, isOn)
        ...
        local activeBg = toggleData.activeBg
        local activeBorder = toggleData.activeBorder
        if not main or not accent then
            return
        end
        local toggleBoxBg = toggleData.toggleBoxBg
        if not toggleBoxBg then
            toggleBoxBg = Color3.new(
                math.clamp(main.R * 0.55, 0, 1),
                math.clamp(main.G * 0.55, 0, 1),
                math.clamp(main.B * 0.55, 0, 1)
            )
            toggleData.toggleBoxBg = toggleBoxBg
        end
        toggleData.container.BackgroundColor3 = toggleBoxBg
        toggleData.container.BackgroundTransparency = 0
        toggleData.container.BorderColor3 = isOn and activeBorder or outline
        ...
        toggleData.knob.Position = isOn and UDim2.new(1, -16, 0.5, -7) or UDim2.new(0, 2, 0.5, -7)
        toggleData.knob.BackgroundColor3 = isOn and font or outline
    end

    local function syncWrapOptionToggles()
        for key, toggleData in pairs(wrapOptionToggles) do
            local isOn = false
    ...
    local function makeThemedToggle(optionKey, labelText, layoutOrder)
        ...
        container.Size = UDim2.fromOffset(72, 54)
        ...
        lbl.Position = UDim2.fromOffset(2, 4)
        ...
        switch.Position = UDim2.new(0.5, -19, 1, -24)
        ...
        container.MouseButton1Click:Connect(function()
            ...
            elseif optionKey == "Favorited" then
                if wrapName and wrapName ~= "" then
                    pcall(function()
                        fav_remote:FireServer(weaponName, wrapName, checked)
                    end)
                end
            end
        end)
    ...
    task.defer(function()
        pcall(styleCosmeticActionButton, fallback)
        pcall(styleCosmeticActionButton, applyBtn)
        pcall(styleCosmeticActionButton, applyAllBtn)
    end)
            ...
            for _, cdata in pairs(cosmetic_lib.Cosmetics or {}) do
                if type(cdata) == "table" and cdata.Image and cdata.Type == "Skin" then
                    if type(cdata.Weapons) == "table" then
                        for _, w in ipairs(cdata.Weapons) do
    ...
    local function resolveCosmeticImage(cname, cosmeticType)
        ...
        if type(base) == "table" then
            imageId = base.Image or base.Icon or base.Thumbnail or base.TextureId
                or base.PreviewImage or base.WrapImage or base.ItemImage or ""
        end
    ...
    local function makeGridTile(parent, displayName, imageId, isActive, palette, onActivated, onRightClick)
        ...
        tile.BorderColor3 = isActive and outline or outline
        tile.Parent = parent
    ...
    local function styleCosmeticActionButton(btn)
        ...
        btn.TextColor3 = font
        if not btn:FindFirstChild("CosmeticBtnCorner") then
            local corner = Instance.new("UICorner")
            corner.Name = "CosmeticBtnCorner"
            corner.CornerRadius = UDim.new(0, 6)
            corner.Parent = btn
        end
        local stroke = btn:FindFirstChild("CosmeticBtnStroke")
        if not stroke then
            stroke = Instance.new("UIStroke")
            stroke.Name = "CosmeticBtnStroke"
            stroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
            stroke.Parent = btn
        end
    ...
    uiState.refreshCosmetics = function()
        ...
        resizeGridCanvas(cosmeticList, cl)
        local _, _, _, outlineTab, activeBgTab, activeBorderTab, activeTextTab, inactiveTextTab = getUiPalette()
        for tn, b in pairs(typeButtons) do
            local active = tn == ctype
            b.TextTransparency = active and 0 or 0.15
            b.BackgroundColor3 = active and activeBgTab or main
            b.BackgroundTransparency = 0
            b.TextColor3 = active and activeTextTab or inactiveTextTab
            b.BorderColor3 = active and activeBorderTab or outlineTab
        end
        local wrapMode = ctype == "Wrap"
        applyAllBtn.TextTransparency = wrapMode and 0 or 0.4
        if syncWrapOptionsBar then syncWrapOptionsBar() end
    end
    ...
    applyAllBtn.MouseButton1Click:Connect(function()
        local ctype = uiState.selectedType or "Skin"
        if ctype ~= "Wrap" then return end
        local selectedWeapon = uiState.selectedWeapon
        if not selectedWeapon then return end
        local pendingByWeapon = uiState.pendingCosmetics[selectedWeapon]
        local wrapName = pendingByWeapon and pendingByWeapon.Wrap
        if not wrapName or wrapName == "" then return end

        for _, weaponName in ipairs(getWeapons()) do
            uiState.pendingCosmetics[weaponName] = uiState.pendingCosmetics[weaponName] or {}
            uiState.pendingCosmetics[weaponName].Wrap = wrapName
            pcall(function()
                fireEquipCosmetic(weaponName, "Wrap", wrapName, getWrapEquipOptions(selectedWeapon))
            end)
        end
    ...
    local function applyCosmeticUiTheme()
        ...
        typeBar.BackgroundColor3 = main
        typeBar.BorderColor3 = outline
        local ctypeNow = uiState.selectedType or "Skin"
        for tn, b in pairs(typeButtons) do
            local active = tn == ctypeNow
            b.BackgroundColor3 = active and accent or main
            b.BackgroundTransparency = 0
            b.TextColor3 = active and font or Color3.new(font.R * 0.85, font.G * 0.85, font.B * 0.85)
        ...
        for _, key in ipairs({ "Inverted", "Favorited" }) do
            ...
            if toggleData then
                ...
                toggleData.activeBg = accent
                toggleData.activeBorder = accent
                toggleData.activeText = font
                toggleData.toggleBoxBg = Color3.new(
                    math.clamp(main.R * 0.55, 0, 1),
                    math.clamp(main.G * 0.55, 0, 1),
                    math.clamp(main.B * 0.55, 0, 1)
                )
        ...
        styleCosmeticActionButton(applyAllBtn)
        for _, d in ipairs(panel:GetDescendants()) do
            if d:IsA("TextButton") and d ~= fallback and d ~= applyBtn and d ~= applyAllBtn then
                local skipBtn = d.Parent == weaponList or d.Parent == cosmeticList or d:IsDescendantOf(wrapToggleRow)
                if not skipBtn then
                    for _, tabBtn in pairs(typeButtons) do
                        if d == tabBtn then
                            skipBtn = true
                            break
                        end
    ...
    getgenv().InstanceApplyCosmeticUiTheme = applyCosmeticUiTheme
end
...
_G.Features.WeaponWireframe = _G.Features.WeaponWireframe or {
    Enabled = false,
    Color = Color3.fromRGB(85, 225, 255),
}
...
function applyultramodaurashadersminecraft()
    ...
    if not (ls.Enabled and ls.Brightness.Enabled) then
        tweenProps.Brightness = 2.1
    end
    if not (ls.Enabled and ls.Ambient.Enabled) then
        tweenProps.Ambient = Color3.fromRGB(20, 23, 35)
    end
    if not (ls.Enabled and ls.OutdoorAmbient.Enabled) then
        tweenProps.OutdoorAmbient = Color3.fromRGB(88, 108, 140)
    end
    if not (ls.Enabled and ls.ShadowSoftness.Enabled) then
        tweenProps.ShadowSoftness = 0.20
    end
    if not (ls.Enabled and ls.EnvironmentDiffuseScale.Enabled) then
        tweenProps.EnvironmentDiffuseScale = 0.95
    end
    if not (ls.Enabled and ls.EnvironmentSpecularScale.Enabled) then
        tweenProps.EnvironmentSpecularScale = 0.98
    end
    if not (ls.Enabled and ls.ColorShiftTop.Enabled) then
        tweenProps.ColorShift_Top = Color3.fromRGB(12, 16, 32)
    end
    if not (ls.Enabled and ls.ColorShiftBottom.Enabled) then
...
_G.Config = _G.Config or {
    Box = {
        ...
        HealthLerpColors = {
            Color1 = Color3.fromRGB(104, 0, 255),
            Color2 = Color3.fromRGB(12, 0, 255),
    ...
    Filled = {
        Enable = false,
        Transparency = 0.7,
        ColorStart = Color3.fromRGB(119, 0, 255),
        ColorEnd = Color3.fromRGB(0, 0, 0),
        Rotation = 0,
        Animated = false,
        Speed = 1,
    },
    Glow = {
        Enable = false,
        Transparency = 0.5,
        ColorStart = Color3.fromRGB(96, 0, 255),
        ColorEnd = Color3.fromRGB(173, 173, 215),
        Rotation = 0,
    },
    TextESP = {
        Names = false,
...
local function CreateBox(player)
    ...
    box.filled_gradient.Rotation = _G.Config.Filled.Rotation
...
local function updatecham(player, box, char)
    ...
    if box.Cham then
        ...
        box.Cham.OutlineTransparency = 1
    end
end
...
local function drawglow(player, box, size, position)
    ...
    box.glow_gradient.Rotation = cfg.Rotation
end
...
local function updateesp(player, dt)
    ...
    local size = Vector2.new(
        math.max(math.floor(boxWidth), 4),
        math.max(math.floor(math.max(boxHeight, 10)), 8)
    )
    local position = Vector2.new(
        math.floor(hrp2d.X - size.X / 2),
        math.floor(math.min(top2d.Y, bottom2d.Y))
    )
...
BoxDepBox:AddToggle('box_filled', {
    ...
    Default = _G.Config.Filled.ColorStart or Color3.fromRGB(119, 0, 255),
    Transparency = false,
    Callback = function(Value)
    ...
    Default = _G.Config.Filled.ColorEnd or Color3.fromRGB(0, 0, 0),
    Transparency = false,
    Callback = function(Value)
...
BoxDepBox:AddToggle('box_glow', {
    ...
    Default = _G.Config.Glow.ColorStart or Color3.fromRGB(96, 0, 255),
    Transparency = false,
    Callback = function(Value)
    ...
    Default = _G.Config.Glow.ColorEnd or Color3.fromRGB(173, 173, 215),
    Transparency = false,
    Callback = function(Value)
...
local function addutilesp(model)
    ...
    local conn = game:GetService("RunService").RenderStepped:Connect(function()
        ...
        local cam = workspace.CurrentCamera
        local pos, visible = worldToEspScreen(rootpart.Position)

        if visible and utilconfig.enable and isselected(model.Name) then
            namedraw.Position = Vector2.new(pos.X, pos.Y - 8)
            namedraw.Visible = true

            if utilconfig.distance then
                local dist = math.floor((cam.CFrame.Position - rootpart.Position).Magnitude)
                distdraw.Text = dist .. " studs"
...
local function refreshutilesp()
    ...
    for _, obj in ipairs(workspace:GetDescendants()) do
        if obj:IsA("Model") and isselected(obj.Name) then
            addutilesp(obj)
        end
    end
end
...
workspace.ChildAdded:Connect(function(child)
    if not utilconfig.enable then return end
    if child:IsA("Model") and isselected(child.Name) then
        task.wait(0.3)
        addutilesp(child)
    end
end)
...
workspace.DescendantAdded:Connect(function(desc)
    if not utilconfig.enable then return end
    if desc:IsA("Model") and isselected(desc.Name) then
        task.wait(0.3)
        addutilesp(desc)
    end
end)
...
_G.LightingSettings = _G.LightingSettings or {
    ...
    SunColor = { Enabled = false, Color = Color3.fromRGB(255, 220, 150) },
...
local function storeorigvalues()
    ...
    origvalues.Brightness = Lighting.Brightness
...
lightbox:AddToggle('lighting_brightness', {
...
lightbox:AddSlider('lighting_brightness_value', {
    Text = 'brightness',
    Min = 0,
    Max = 10,
    Default = 3,
...
getgenv().crosshair = {
    ...
    crosshair_color = Color3.fromRGB(0, 85, 255),
    
    color1 = Color3.fromRGB(255, 0, 128),
    color2 = Color3.fromRGB(0, 186, 255),
    color3 = Color3.fromRGB(128, 0, 255),
    gradient_rotation = 0,
...
local textChars = {}
local text1 = "instance"
for i = 1, #text1 do
    local char = Drawing.new("Text")
    char.Size = 13
    char.Font = 2
    char.Outline = true
    char.Text = text1:sub(i, i)
    char.Color = Color3.new(1, 1, 1)
    textChars[i] = char
end

local text2 = ".elf"
local text2Offset = #text1
for i = 1, #text2 do
    local char = Drawing.new("Text")
    char.Size = 13
    char.Font = 2
    char.Outline = true
    char.Text = text2:sub(i, i)
    char.Color = Color3.new(1, 1, 1)
    textChars[text2Offset + i] = char
end
...
local ammoCharWidths = {}
do
    local chars = "0123456789 /"
    for i = 1, #chars do
        local c = chars:sub(i, i)
        local tmp = Drawing.new("Text")
        tmp.Size = 13
        tmp.Font = 2
        tmp.Text = c
        ammoCharWidths[c] = tmp.TextBounds.X
        tmp:Remove()
    end
end
...
local function measureAmmoText(str)
    local w = 0
    for i = 1, #str do
        w = w + (ammoCharWidths[str:sub(i, i)] or 7)
    end
    return w
end

local totalChars = #text1 + #text2
...
Run.PostSimulation:Connect(function()
    ...
    local lineReach = cfg.show_lines and (cfg.radius + length) * scale or 0
    local textY = pos.Y + (cfg.show_lines and (cfg.radius + cfg.length + 14) or 15)

    if cfg.show_watermark then
        local totalWidth = 0
        for i = 1, totalChars do
            totalWidth = totalWidth + textChars[i].TextBounds.X
        end

        local x = pos.X - totalWidth / 2
        local curx = x

        for i = 1, totalChars do
            local charProgress = (i - 1) / (totalChars - 1)
            local gradcolor2 = gradcolor(charProgress, cfg.gradient_rotation, cfg.color1, cfg.color2, cfg.color3)

            textChars[i].Visible = true
            textChars[i].Position = Vector2.new(curx, textY)
            textChars[i].Color = gradcolor2
    ...
    if cfg.show_ammo then
        ...
        if ammoText ~= prevAmmoText then
            ...
            prevAmmoWidth = measureAmmoText(ammoText)
    ...
    if cfg.show_lines then
        ...
        local crosshairColor = cfg.crosshair_color
...
local weatherui = {
    targetgroup = Tabs.World:AddLeftTabbox()
}
weatherui.raintab = weatherui.targetgroup:AddTab('rain')
weatherui.snowtab = weatherui.targetgroup:AddTab('snow')
weatherui.lightraintab = weatherui.targetgroup:AddTab('light rain')
...
local function applyparticle(p, cfg)
    ...
    p.Size = NumberSequence.new{
        NumberSequenceKeypoint.new(0, cfg.size_min),
        NumberSequenceKeypoint.new(1, cfg.size_max)
    }
...
RunService.PostSimulation:Connect(function()
    local p = cam.CFrame.Position + heightoffset
    for key, part in next, weatherparts do
        part.CFrame = CFrame.new(p)
    end
end)

local function makeweathertab(tab, key)
    local cfg = weathercfgs[key]

    tab:AddToggle("weatherenabled_" .. key, {
        Text = "enable",
        Default = false,
        Callback = function(value)
            cfg.enabled = value
            if value then build(key) else destroy(key) end
        end
    }):AddColorPicker("weathercolor_" .. key, {
        Default = cfg.color,
        Title = "color",
        Callback = function(value)
            cfg.color = value
    ...
    tab:AddSlider("weatherrate_" .. key, {
        Text = "rate",
        Default = 100,
        Min = 0,
        Max = 100,
        ...
        Callback = function(value)
            cfg.rate = value
            if weatherparticles[key] then
                weatherparticles[key].Rate = cfg.base_rate * (value / 100)
            end
        end
    })

    tab:AddDualSlider("weatherspeed_" .. key, "weatherspeed2_" .. key, {
        Text = "speed min",
        Default = cfg.speed_min,
        Min = 0,
        Max = 200,
        Rounding = 1,
        Suffix = "",
    ...
    tab:AddDualSlider("weathersize_" .. key, "weathersize2_" .. key, {
        Text = "size min",
        Default = cfg.size_min,
        Min = 0.1,
        Max = 50,
        Rounding = 2,
        Suffix = "",
    ...
    tab:AddDualSlider("weatheropacity_" .. key, "weatheropacity2_" .. key, {
        Text = "opacity min",
        Default = math.floor((1 - cfg.transparency_min) * 100),
        Min = 0,
        Max = 100,
        Rounding = 0,
        Suffix = "%",
        Callback = function(value)
            cfg.transparency_min = 1 - (value / 100)
            refresh(key)
        end
    }, {
        Text = "opacity max",
        Default = math.floor((1 - cfg.transparency_max) * 100),
        Min = 0,
        Max = 100,
        Rounding = 0,
        Suffix = "%",
        Callback = function(value)
            cfg.transparency_max = 1 - (value / 100)
            refresh(key)
        end
    })

    tab:AddSlider("weatherspread_" .. key, {
        Text = "spread",
        Default = cfg.spread,
        Min = 0,
        Max = 180,
    ...
    tab:AddSlider("weatherbrightness_" .. key, {
        Text = "brightness",
        Default = cfg.brightness * 100,
        Min = 0,
        Max = 500,
        Rounding = 0,
        Suffix = "%",
        Callback = function(value)
            cfg.brightness = value / 100
            refresh(key)
        end
    })

    tab:AddSlider("weatheremission_" .. key, {
        Text = "light emission",
        Default = cfg.light_emission * 100,
        Min = 0,
        Max = 100,
        Rounding = 0,
        Suffix = "%",
        Callback = function(value)
            cfg.light_emission = value / 100
            refresh(key)
        end
    })
end

makeweathertab(weatherui.raintab, "rain")
makeweathertab(weatherui.snowtab, "snow")
makeweathertab(weatherui.lightraintab, "lightrain")

weatherui.snowtab:AddSlider("weathersnowamount", {
    Text = "snow amount",
    Default = weathercfgs.snow.amount,
    Min = 1,
    Max = 1000,
    Rounding = 0,
    Suffix = "",
    Callback = function(value)
        weathercfgs.snow.amount = math.clamp(value, 1, 1000)
        refresh("snow")
    end
})
...
local function syncMenuOverlayUi()
    ...
    if Library and Library.KeybindFrame then
        if open then
            ...
            local showList = Options.KeybindVisible and Options.KeybindVisible.Value
            Library.KeybindFrame.Visible = showList == true
        end
    end
...
MenuGroup:AddToggle('KeybindVisible', {
    ...
    Default = false,
    Callback = function(Value)
        if isInstanceMenuOpen() then return end
...
(function()
    ...
    p.CharacterAdded:Connect(function()
        ...
        serverRigCacheAt = 0
        task.wait(1.5)
        if Toggles.AnimAutoRespawn and Toggles.AnimAutoRespawn.Value and Toggles.AnimEnabled and Toggles.AnimEnabled.Value then
            getgenv()._animCmd = "Play"
        end
    end)
end)();
...
local function hasTargetTextureInDescendants(inst)
    if not inst then
        return false
    end
    if instanceHasTargetTexture(inst) then
        return true
    end
    for _, child in ipairs(inst:GetDescendants()) do
        if instanceHasTargetTexture(child) then
            return true
        end
    end
    return false
...
local function instanceHasTargetTexture(inst)
    ...
    if inst:IsA("Texture") then
        for _, prop in ipairs({"Texture", "ColorMap", "ColorMapContent"}) do
            if matchAssetValue(getInstProperty(inst, prop)) then
                return true
            end
        end
    elseif inst:IsA("Decal") then
...
local function recordTargetTexture(inst)
    if not inst or targetTextureOrig[inst] or not instanceHasTargetTexture(inst) then
        return
    end
...
local function restoreTargetTexture(inst)
    ...
    if inst:IsA("Texture") or inst:IsA("Decal") or inst:IsA("SurfaceAppearance") then
        if orig.Texture ~= nil then
            setInstProperty(inst, "Texture", orig.Texture)
        end
        if orig.TextureId ~= nil then
            setInstProperty(inst, "TextureId", orig.TextureId)
        end
        if orig.TextureContent ~= nil then
            setInstProperty(inst, "TextureContent", orig.TextureContent)
        end
        if orig.ColorMap ~= nil then
            setInstProperty(inst, "ColorMap", orig.ColorMap)
        end
        if orig.ColorMapContent ~= nil then
            setInstProperty(inst, "ColorMapContent", orig.ColorMapContent)
        end
        if orig.Albedo ~= nil then
            setInstProperty(inst, "Albedo", orig.Albedo)
        end
        if orig.NormalMap ~= nil then
            setInstProperty(inst, "NormalMap", orig.NormalMap)
        end
        if orig.MetalnessMap ~= nil then
            setInstProperty(inst, "MetalnessMap", orig.MetalnessMap)
        end
        if orig.RoughnessMap ~= nil then
            setInstProperty(inst, "RoughnessMap", orig.RoughnessMap)
        end
        if orig.Transparency ~= nil then
            setInstProperty(inst, "Transparency", orig.Transparency)
        end
    end
...
local function degradeTextureInstance(inst)
    if not inst or not instanceHasTargetTexture(inst) then
        return
    end
    recordTargetTexture(inst)
    if inst:IsA("Texture") then
        setInstProperty(inst, "Transparency", 1)
        if matchAssetValue(getInstProperty(inst, "Texture")) then
            setInstProperty(inst, "Texture", "")
        end
        if matchAssetValue(getInstProperty(inst, "ColorMap")) then
            setInstProperty(inst, "ColorMap", "")
        end
        if matchAssetValue(getInstProperty(inst, "ColorMapContent")) then
            setInstProperty(inst, "ColorMapContent", "")
        end
    elseif inst:IsA("Decal") then
        setInstProperty(inst, "Transparency", 1)
        if matchAssetValue(getInstProperty(inst, "Texture")) then
        ...
        if matchAssetValue(getInstProperty(inst, "TextureId")) then
            setInstProperty(inst, "TextureId", "")
        end
        if matchAssetValue(getInstProperty(inst, "TextureContent")) then
            setInstProperty(inst, "TextureContent", "")
        end
    elseif inst:IsA("SurfaceAppearance") then
        if matchAssetValue(getInstProperty(inst, "ColorMap")) then
            setInstProperty(inst, "ColorMap", "")
        end
        if matchAssetValue(getInstProperty(inst, "Albedo")) then
            setInstProperty(inst, "Albedo", "")
        end
        if matchAssetValue(getInstProperty(inst, "ColorMapContent")) then
            setInstProperty(inst, "ColorMapContent", "")
        end
        if matchAssetValue(getInstProperty(inst, "NormalMap")) then
            setInstProperty(inst, "NormalMap", "")
        end
        if matchAssetValue(getInstProperty(inst, "MetalnessMap")) then
            setInstProperty(inst, "MetalnessMap", "")
        end
        if matchAssetValue(getInstProperty(inst, "RoughnessMap")) then
            setInstProperty(inst, "RoughnessMap", "")
        end
    end
end
...
local function processTargetTextureInstance(inst)
    ...
    if instanceHasTargetTexture(inst) then
        degradeTextureInstance(inst)
    end
    for _, child in ipairs(inst:GetDescendants()) do
        if instanceHasTargetTexture(child) then
            degradeTextureInstance(child)
        end
    end
end
